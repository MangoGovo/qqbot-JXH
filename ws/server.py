...
from .request_quote import send_post_request
...
async def process_q_command(ws, data):
    message_content = data.get("message", [])
    if message_content[0].get("type") == "reply":
        q_data = await get_message(ws, message_content[0]["data"]["id"])
        ...
        if q_message_content[0].get("type") == "reply":
            q_message_mention_data = await get_message(
                ws, q_message_content[0]["data"]["id"]
            )  # /q 引用的消息引用的消息
...
async def add_scheduled_job(message):
    ...
    job_type, time_str, group_id, job_message = (
        parts[0],
        parts[1],
        parts[2],
        " ".join(parts[3:]),
    ...
    if job_type == "每天":
        schedule.every().day.at(time_str).do(
            lambda: asyncio.run_coroutine_threadsafe(
                send_group_message(int(group_id), job_message, tag), loop
            )
        ).tag(tag)
    elif job_type == "单次":
        schedule.every().day.at(time_str).do(
...
def schedule_job(job):
    ...
    tag = job.get(
        "tag", f"{job['type']}_{job['time']}_{group_id}_{random.randint(1, 10000)}"
    )
    if job["type"] == "每天":
        logger.info(job["type"])
        schedule.every().day.at(job["time"]).do(
            lambda: asyncio.run_coroutine_threadsafe(
                send_group_message(group_id, job_message, tag), loop
            )
        ).tag(tag)
    elif job["type"] == "单次":
        logger.info(job["type"])
    ...
    while True:
        ...
        if data.get("message_type") == "group" and data.get("raw_message"):
            ...
            if message == "/test":
                ...
                msg = await process_admin_command(
                    ws, message[7:], ats, user_id, is_me, group_id
                )
            elif message == "/reload":
